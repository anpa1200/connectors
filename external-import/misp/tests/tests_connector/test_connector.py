...
class TestProcessEventsAttributeTimestampFiltering:
    ...
    @freeze_time("2026-01-01 00:00:00")
    def test_attribute_timestamp_passed_to_api_search(
        self, mock_opencti_connector_helper, mock_py_misp
    ):
        """When state has last_attribute_timestamp, it's passed to search_events as attribute_timestamp."""
        config_dict = deepcopy(minimal_config_dict)
        config_dict["misp"]["datetime_attribute"] = "publish_timestamp"
        config_dict["misp"]["attribute_timestamp_filtering"] = True
        connector = fake_misp_connector(config_dict)
        ...
        threshold = 1500
        initial_state = {
            "last_event_date": "2025-12-01T00:00:00+00:00",
            "last_attribute_timestamp": threshold,
        }
        ...
        with (
            ...
            patch.object(connector, "work_manager") as mock_wm,
            patch.object(connector, "client_api") as mock_api,
            patch.object(connector, "converter"),
            patch.object(connector, "batch_processor"),
            patch.object(connector, "_process_bundle_in_batch"),
        ):
            mock_helper.get_state.return_value = initial_state
            mock_helper.metric = MagicMock()
            mock_wm.get_state.return_value = dict(initial_state)
            mock_wm.update_state.side_effect = lambda **kw: None
            mock_api.search_events.return_value = iter([])

            connector.process_events()

            # Verify attribute_timestamp was passed to search_events
            call_kwargs = mock_api.search_events.call_args
            assert "attribute_timestamp" in call_kwargs.kwargs or (
                call_kwargs[1] and "attribute_timestamp" in call_kwargs[1]
            )
            passed_ts = call_kwargs.kwargs.get("attribute_timestamp") or call_kwargs[
                1
            ].get("attribute_timestamp")
            expected_dt = datetime.fromtimestamp(threshold, tz=timezone.utc)
            assert passed_ts == expected_dt
