...
class Misp:
    ...
    def process_events(self) -> str | None:
        ...
        try:
            ...
            # Attribute-level timestamp filtering: use the MISP API parameter
            # to narrow results to events with recently modified attributes,
            # and track the timestamp for client-side filtering.
            ...
            if attribute_ts_filtering:
                ...
                if since_attr_timestamp is not None:
                    filter_params["attribute_timestamp"] = datetime.fromtimestamp(
                        int(since_attr_timestamp), tz=timezone.utc
                    )
                    self.logger.info(
